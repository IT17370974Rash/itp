...
CREATE TABLE `allocate` (
  ...
  `vehicleno` varchar(9) DEFAULT NULL,
  `product` int(11) DEFAULT NULL,
  `date` date DEFAULT NULL,
  `distanse` int(11) DEFAULT NULL,
  PRIMARY KEY (`empno`),
  KEY `fk_allocate_transport_idx` (`vehicleno`,`product`,`date`),
  CONSTRAINT `fk_allocate_empno` FOREIGN KEY (`empno`) REFERENCES `permenant_employee` (`empno`),
  CONSTRAINT `fk_allocate_transport` FOREIGN KEY (`vehicleno`, `product`, `date`) REFERENCES `transport` (`vehicleno`, `productid`, `date`)
